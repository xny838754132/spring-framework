...
public class DefaultListableBeanFactory extends AbstractAutowireCapableBeanFactory
	...
	// 所有 BeanDefinition 定义信息 按照名字对应的对应信息
	...
	// Spring  中按照类型得到组件中的一个底层池
	private final Map<Class<?>, String[]> singletonBeanNamesByType = new ConcurrentHashMap<>(64);

	/** L,ist of bean definition names, in registration order. */
	// 保存所有 Bean 定义的名字 ，每注册一个 Bean 都有
	...
	private String[] doGetBeanNamesForType(ResolvableType type, boolean includeNonSingletons, boolean allowEagerInit) {
		...
		// Check all bean definitions. 因为 Spring没有直接保存 Class -- Bean 名字的对应信息 ； 只能遍历所有的BeanName，拿出他们 BeanName 的定义信息，再看是否是我指定的类型
		for (String beanName : this.beanDefinitionNames) {
			// Only consider bean as eligible if the bean name is not defined as alias for some other bean.
			if (!isAlias(beanName)) { // 判断是否是别名
				try {
					...
					if (!mbd.isAbstract() && (allowEagerInit ||
						...
						boolean isFactoryBean = isFactoryBean(beanName, mbd); // 首先判断是否是 FactoryBean
	...
	public void preInstantiateSingletons() throws BeansException {
		...
		// 创建出所有的单实例 Bean  Trigger initialization of all non-lazy singleton beans...
		for (String beanName : beanNames) {
			RootBeanDefinition bd = getMergedLocalBeanDefinition(beanName); // 开始解析文件的时候每一个bean标签 被解析封装成一个 BeanDefinition
			if (!bd.isAbstract() && bd.isSingleton() && !bd.isLazyInit()) { // 校验不是抽象Bean 并且 是单例的 不是懒加载的
				if (isFactoryBean(beanName)) { // 如果是 FactoryBean 则执行下面的逻辑
					Object bean = getBean(FACTORY_BEAN_PREFIX + beanName); // 可以得到自己定义的组件
