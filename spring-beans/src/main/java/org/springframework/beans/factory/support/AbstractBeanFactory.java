...
public abstract class AbstractBeanFactory extends FactoryBeanRegistrySupport implements ConfigurableBeanFactory {
	...
	/**
	 * 保存所有的 BeanPostProcessor BeanPostProcessors to apply.
	 */
	...
	/**
	 * Names of beans that have already been created at least once. 已经创建的 BeanName 集合
	 */
	...
	protected <T> T doGetBean(
		...
		String beanName = transformedBeanName(name); // 转换 Bean 的名字
		Object beanInstance;

		// 先检查但实例 Bean 的缓存 Eagerly check singleton cache for manually registered singletons.
		...
		if (sharedInstance != null && args == null) {
			...
			// 拿到整个 BeanFactory 的父工厂；如果没有父工厂，则继续；从父工厂先尝试获取组件； Check if bean definition exists in this factory.
			...
			try {
				...
				String[] dependsOn = mbd.getDependsOn(); // 获取依赖的所有组件
				if (dependsOn != null) {
					for (String dep : dependsOn) { // 当前 Bean 是否有依赖其他 Bean
						...
						try {
							getBean(dep); // 依赖了其他 Bean ，就先获取其他的 Bean
				...
				if (mbd.isSingleton()) {
					sharedInstance = getSingleton(beanName, () -> {
						try {
							return createBean(beanName, mbd, args); // 创建 Bean 的实例
	...
	@SuppressWarnings("unchecked") // 适配器模式
	...
	protected boolean isTypeMatch(String name, ResolvableType typeToMatch, boolean allowFactoryBeanInit)
		...
		// 检查是否已经有这个 BeanName 的单例对象，有了直接用对象的Class，没有可以最后一次绝对这个组件的Class Check manually registered singletons.
		Object beanInstance = getSingleton(beanName, false);
		...
		// 通过后置处理器，可以返回自定义的 If we couldn't use the target type, try regular prediction.
